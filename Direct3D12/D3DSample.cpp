...
void D3DSample::Render()
{
    ...
    // 쉐도우 맵 텍스쳐 렌더링 파이프라인에 묶기
    CD3DX12_GPU_DESCRIPTOR_HANDLE ShadowHeapAddress(m_TextureDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
    ShadowHeapAddress.Offset(m_ShadowMapHeapIndex, m_CbvSrvUavDescriptorSize);
    m_CommandList->SetGraphicsRootDescriptorTable(5, ShadowHeapAddress);
...
void D3DSample::BuildShadowMap()
{
    ...
    m_ShadowMapSicssorRect = { 0, 0, (int)m_ShadowMapWidth, (int)m_ShadowMapHeight };
...
{
    // SRV Heap
    // 텍스처 맵 갯수 + 스카이박스 텍스처 갯수 + 쉐도우 맵 텍스쳐 갯수
...
void D3DSample::RenderSceneToShadowMap()
{
    ...
    m_CommandList->RSSetScissorRects(1, &m_ShadowMapSicssorRect);
